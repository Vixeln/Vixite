...
const calculator = computed<Calculator | GraphingCalculator | undefined>(
	(prevValue) => {
		if (desmosIsLoaded.value && domRef.value && Desmos) {
			...
			const constructorMap: Record<
				CalculatorType,
				Constructor<Calculator | GraphingCalculator>
			> = {
			...
			const temp = constructorMap[selectedGraph.value.type](
				domRef.value,
